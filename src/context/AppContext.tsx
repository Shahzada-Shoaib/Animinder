...
export const AppProvider = ({children}: {children: ReactNode}) => {
  ...
  const loadUserChats = (userId: string) => {
    ...
    if (userId && userId !== 'user1') {
      const unsubscribe = getUserChats(userId, (userChats) => {
        setChats(userChats);
      });
  ...
  const createChat = async (otherUserId: string): Promise<Chat | null> => {
    if (!currentUser.id || currentUser.id === 'user1' || !otherUserId) {
      return null;
    }
